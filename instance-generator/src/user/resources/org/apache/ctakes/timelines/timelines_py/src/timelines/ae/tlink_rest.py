...
from .cnlp_rest import get_dataset, initialize_model
...
async def startup_event(tlink_path):
    ...
    initialize_model(app, tlink_path)
...
def rest():
    ...
    uvicorn.run(
        "timelines.ae.tlink_rest:app", host="0.0.0.0", port=args.port, reload=True
    )
