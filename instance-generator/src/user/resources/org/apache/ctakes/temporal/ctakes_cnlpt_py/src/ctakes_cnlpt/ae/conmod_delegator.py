...
async def conmod_caller(cas, text, event_mentions, offsets):
    e_doc = EntityDocument(doc_text=text, entities=offsets)
    # async with sem:
    conmod_output = await negation_rest.process(e_doc)
    i = 0
    for e in event_mentions:
        # -1 represents that it had happened, 1 represents that it is negated
        e.contextualModality = conmod_output.statuses[i] * -1
        i += 1
    ...
    def process(self, cas):
        ...
        while i < len(sentences):
            if len(sentence_events[i]) > 0:
                ...
                asyncio.run(conmod_caller(cas, sentences[i].get_covered_text(), sentence_events[i], offsets))
            i += 1
