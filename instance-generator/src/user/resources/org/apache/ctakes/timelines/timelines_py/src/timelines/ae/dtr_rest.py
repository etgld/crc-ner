...
app = FastAPI()
logger = logging.getLogger("DocTimeRel Processor")
...
async def startup_event(dtr_path):
    initialize_model(app, dtr_path, cuda=False, batch_size=64, mode="Hf")
...
def rest():
    ...
    uvicorn.run("timelines.ae.dtr_rest:app", host="0.0.0.0", port=args.port, reload=True)
