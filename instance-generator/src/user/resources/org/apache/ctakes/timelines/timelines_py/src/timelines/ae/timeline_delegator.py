...
class TimelineDelegator(cas_annotator.CasAnnotator):
    ...
    def process(self, cas):
        ...
        positive_sentence_events = [
            [*filter(is_positive, sent_events)]
            for sent_events in get_covered_list(sentences, event_mentions)
        ]
        ...
        #             "Processing cnlp-transformers timelines on sentence",
        #             str(i),
        #             "of",
        #             str(len(sentences)),
        #             "...",
        #         )
        #         event_offsets = get_windowed_offsets(
        #             positive_sentence_events[i], sentences[i].begin
        #         )
        #         token_offsets = get_windowed_offsets(
        #             sentence_tokens[i], sentences[i].begin
        #         )
        #         asyncio.run(
        #             self.temporal_caller(
        #                 cas, sentences[i], sentence_events[i], token_offsets
        #             )
        #         )
        #     i += 1
    ...
    async def temporal_caller(self, cas, sentence, event_mentions, token_offsets):
        ...
        for e in temporal_result.events:
            for ee in e:
                ...
                event_mention = self.get_or_create_event_mention(
                    cas,
                    event_mentions,
                    sentence.begin + first_token_offset[0],
                    sentence.begin + last_token_offset[1],
                )
                event = event_creator.create_event(cas=cas, dtr=ee.dtr)
        ...
        for r in temporal_result.relations:
            for rr in r:
                ...
                tlink = create_relation(
                    cas=cas,
                    relation_type=self.tlink_type,
