...
final public class Summarizer extends JCasAnnotator_ImplBase {
...
// TODO - add data structures for keeping track of cohort level and patient level instances
// probably Map( cohort : String , Map( patient : String, Set( Pair( timex : String, CUIs : String ) ) ) )
   ...
   public void process( final JCas jCas ) throws AnalysisEngineProcessException {
      _docCount++;
      // TODO - update cohort and patient level datastructures with
      // data from treatment instances in the CAS - i.e. (normalized ?) timex, CUI(s?)
      ProgressManager.getInstance().updateProgress( Long.valueOf( _docCount ).intValue() );
   }

   public void collectionProcessComplete() throws AnalysisEngineProcessException {
      // TODO - here we would write to CSV or whatever format for scoring
      // Or given the scoring script as part of the PBJ package, either via PythonRunner
      // or even within this process, provided a path to the gold CSV, we can score automatically
